...
function genCode(handler: FairyEditor.PublishHandler) {
    ...
    for (let i = 0; i < classCnt; i++) {
        ...
        classInfo.className = classInfo.className.replace("{codePkgName}", codePkgName);
        let references = classInfo.references;
        let refCount = references.Count;
        if (refCount > 0) {
            for (let j = 0; j < refCount; j++) {
                references.set_Item(j, references.get_Item(j).replace("{codePkgName}", codePkgName));
            }
        }
        ...
        for (let j = 0; j < memberCnt; j++) {
            ...
            if (memberInfo.group === 0) {
                memberInfo.type = memberInfo.type.replace("{codePkgName}", codePkgName);
            }
        }
    }
